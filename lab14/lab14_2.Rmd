...
A quick review:
Genotype is underlying DNA sequences/variants of genes
Phenotype is traits manifesting as a result of genotype.
Homozygous means alleles same, heterozygous means alleles different.
Remember the Principle of Segregation and Principle of Independent Segregation.

Qualitative traits: discrete variation (color, shape)
Quantitative traits: continuous variation (polygenic traits)

Quantitative trait locus (QTL) analysis: combines DNA marker and phenotypic trait data to locate and characterize genes that influence quantitative traits. You have to try and find association between molecular markers and phenotypic traits.

Logarithm of Odds (LOD) Score: refers to a numerical result when estimating whether a gene and a phenotype are linked to one another. A high LOD score means a gene indicatied in the region links to the phenotype you are investigating.
...
You read the file, filename, and specify the genotype you have. We have CC, CB, and BB. The alelles are C and B.
...
summary(sug)
```
This tells you how many individuals are recorded in the data set and how many phenotypes are recorded. The percentage of phenotypes phenotyped (some will be missing, which will not be indicated here). This is very good data as most have been phenotyped and at a high percentage.
We also have the total gene marker number. For each chromosome, it tells you the amount of markers. It also says the distribution for each genotype.
...
nmar(sug)    # No. markers on each chromosome in short matrix
```
...
plot(sug)
```
The first is missing data points, then genetic map, then 4 quantitative traits in histograms, then sex (only have 1 gender), and then mouse ID (number mouses recorded). You don't needall of these, so you can plot the specific things you want to see.

Individual parts of the above plot may be obtained as follows.
```{r}
#?plotMissing
plotMissing(sug, main="") # summarise missing data in plot
plotMissing(sug, main="", reorder=1) # reorder means we are using the first phenotype (e.g. bp) as the standard.
#summary(sug)
#drop.nullmarkers(sug) # if you don't have info for a marker, drop it!

#?plotMap
plotMap(sug)
plotMap(sug,show.marker.names=TRUE)

#?plotPheno -- plots phenotype you are interested in
...
```{r}
corrplot <- iplotCorr(sug$pheno, chartOpts=list(height=300, width=600, scatcolors="pink")) # must assign to an object
...
out.em <- scanone(sug) # when you calculate associations, it will use the first phenotype (e.g. BP)
...
summary(out.em2)
```
High LOD means that c7.loc45 associates highly with blood pressure.


Alternatively, we can give a threshold, e.g., to only see those chromosomes with LOD > 3. About 1:1000 probability you will get this phenotype (this means that the genotype is highly associated with phenotype)
```{r}
