...
```
One note: one of the rows has a "\\" as a value, which is why it's there in the NA concatenation.
...
Map shapes or colors to another categorical variable.
```{r}
p + 
  geom_point(aes(color=thermoregulation), size=1.75)
```
Compare it to:
```{r}
p + 
...
```
There are so many colors you can use... but, beware of the colors you choose due to colorblindness.
...
homerange %>% 
  ...
  labs(title = "Observations by Taxon in Homerange Data",
       ...
       fill = "Trophic Guild")
```
It's a little tricky to count the number of carnivores here...
...
homerange %>% 
  ...
  geom_boxplot()
```
There are different groups, but note that nothing is colored in! This may or may not be appropriate to the graph you want to make...
...
Group and fill work similar ways, but fill will create color.
