...
``` 
Filter through the data frame "fish" to lake ID with "AL". We want to pull out all observations for which lakeid is "AL", so 4033 observations decreases. Now we are at 383 observations!
...
Using the `!` operator allows for the exclusion of specific observations. This is the "not" operator.
```{r}
filter(fish, lakeid != "AL")
```
We took out all observations that had to do with lakeid == AL.
...
```
We filtered fish inside the length column. Looking through the observations, length is either 167 or 175... but not anything else. WE filtered out everything else.
...
```
All of the observations are between 2.5 and 2.55 but did not include the endpoints.

You can also extract observations "near" a certain value but you need to specify a tolerance. Super helpful to find values around what you stated.  
```{r}
filter(fish, near(radii_length_mm, 2, tol = 0.2))
```
These are the values below and above 2
...
```
Feel free to check your coding by storing your dataframes in the environment.

What's the difference between `filter()`, `select()`, and 'subset()'?
They use the same organizational philosophy.. but filter/select is so much better.
...
```
This looks for rows that has both conditions being met to be satisfied!

```{r}
filter(fish, lakeid == "AL" & length == 350 & radii_length_mm >= 3)
```

Notice that the `|` operator generates a different result. This is giving us the "or", the "either". So, we will get either lakeid == "AL" or length greater than 350. Basically, there will be other lakes with length greater than 350 that are not AL. Then, you will also get AL lake id without the length being greater than 350.
```{r}
filter(fish, lakeid == "AL" | length > "350")
```
...
In this case, we filter out the fish with a length over 400 and a scale length over 11 or a radii length over 8. You can combine operators!
```{r}
...
```{r}
mammal_compare <- select(mammals, "genus", "mass", "gestation", "newborn")
mammal_compare
```

```{r}
newborns <- filter(mammal_compare, genus == "Lophocebus" | genus == "Erythrocebus" | genus == "Macaca")
newborns
summary(newborns)
```
```{r}
filter(newborns, newborn <= 390)
```
The Macaca has the smallest newborn size!

**Alternatively...**
```{r}
primates_genus <- filter(mammals, genus %in% c("Lophocebus","Erythrocebus", "Macaca"))
primates_genus
view(primates_genus)
```
The Macaca maura had the smallest newborn mass.
