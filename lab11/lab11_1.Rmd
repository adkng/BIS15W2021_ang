...
deserts %>% 
  ...
  summarise(n=n())
```
We want to know how many times this species was collected/observed during these years. The columns are tidy, so it is not super easy to read.
```{r}
...
deserts %>% 
  ...
  labs(title = "Number of samples for species DM",
       ...
       fill = "n")
```
We have two separate lines, and they are also colored by the same variable. Note the years are broken up in increments of 5... If we look at the deserts data, we see that 'year' is treated as a numeric... but, is year really a numeric? Would you sum these across? No! Years are actually *factors*. Once you reassign the data type to a factor, now all the years show up and you can easily see each annual observation.
...
In practice, we probably shouldn't change the number of bins unless the graph starts getting funky.
...
```
Note, each bar is a bin and has a black line across it to make it "pop"! The `color = ` changes the color of the outline of the bar (they can be black, grey, or not there!). `Alpha = ` refers to transparency. The current alpha is set to 0.6... if setting to 0.9, it makes the plot a lot darker. It's preferable to keep it a little transparent so your readers can see the measurements behind the graph.

Let's play with the colors. This shows all 657 of R's built-in colors. Notice that `color` and `fill` do different things!
```{r}
#grDevices::colors() #wow! so many colors...
```
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.6, color = "black", fill = "steelblue")+
  labs(title = "Distribution of Homerange")
```
You can play around with the bin number here. But, typically it's okay to leave it as is unless we know something about the data that would make the graph better.
...
glimpse(homerange)
```
Notice that a lot of these characters are continuous... but, what if we want to make the continuous variables into categorical variables? We can't just compile them all into categorical values unless the data type is very close.

For example, with a variety of heights, you want to break them up into "short", "medium", and "tall". You should make clean breaks on a continuous variable.
...
The range is quite high! How would you break this up?
```{r}
summary(homerange$log10.mass)
```
Notice that this gives you a lot of information. We can use this to create our "breaks".
...
glimpse(homerange)
```
In the homerange data, we are mutating (adding a new column) called mass_category with three categories: small, medium, and large. We are turning a continuous variable into categorical variables. We will use the `case_when()` to categorize the different values.
...
table(quartiles)
```
Wow! What an easy-to-use way to get quartiles when breaking something down.
