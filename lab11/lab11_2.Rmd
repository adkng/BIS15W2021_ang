...
p +
  theme_linedraw() +
  theme(legend.position = '', #gets rid of the legend entirely
        axis.text.x = element_text(angle = 60, hjust=1))+
  labs(title = "Observations by Taxon in Homerange Data",
       x = NULL,
       y= "n",
       fill= "Trophic Guild") # `fill = "" is the new legend title
```
...
```{r}
display.brewer.pal(8,"PRGn") # you want an eight-color palette from x
```
...
view(colors) # tells you how many colors and best use
```
...
```{r}
my_palette <- paletteer::paletteer_d("ggsci::hallmarks_light_cosmic")
```
...
```{r}
p2+ facet_wrap(~taxon) # i want you to make a wrap by/`~` taxon (a categorical variable)
```
```{r}
p2 +
  facet_wrap(~class) # do not give it a categorical variable with too many things
```

## Faceting: `facet_grid()`
As you can imagine, there are lots of options for faceting. Another useful facet type is `facet_grid`. 
```{r}
p2+facet_grid(thermoregulation~class) # class on top, thermoregulation on the side
```
...
## Printing
There are many ways to export things out of R... here's how you do it!
```{r}
pdf("homerange.pdf") # we are telling it the name 
p2 # this is the plot we want to export
dev.off() # this means we close the pdf
```
