...
library("tidyverse")
```
We have just called the tidyverse.
...
Imagine you wanted to look at someone else's data... R has no problem reading Excel formats! Most of Excel is related to aesthetics. However, "csv" is able to be read by many programs and plays a lot nicer with R. If you want to turn a spreadsheet in Excel into something usable for R: File > Save As > CSV.
...
Session > Set Working Directory > Choose working directory > Open
Setwd()


## Load the data
Here we open a .csv file. Since we are using the tidyverse, we open the file using `read_csv()`. `readr` is included in the tidyverse set of packages. We specify the package and function with the `::` symbol. This becomes important if you have multiple packages loaded that contain functions with the same name. There is `read.csv.`, but it's older and we won't use it honestly.  
...
```
This will also let you know if there are any errors in your import.
...
glimpse(hot_springs)
```
Remember, the <carats> indicate the class of data. For example, <dbl> is a double floating point number.

We don't want each instance of scientist to be treated differently... so let's change the class of scientist!
...
What is the difference between a character and a factor?
A `character` is... a string of characters. '1' and '7'.
A `factor` is... in possession of specific levels. '17'.
...
**Fun fact:** If you want to remove a certain object from the global environment, then just use the `rm()` function.
```{r}
rm(fish)
```
...
table(fish$lakeid)
```
Here is the number of observations for each lake ID.

```{r}
table(fish$length)
```
Continuous variables give you a weird output... works best for categorical/discrete things.


We can also click on the `fish` data frame in the Environment tab or type View(fish). This brings up a spreadsheet.
```{r}
View(fish)
```

## Subset
Subset is a way of pulling out observations that meet specific criteria in a variable. For example, what if we wanted to see fish with tails less than 100.  
```{r}
little_fish <- subset(fish, length<=100) #creates the subset, see environment! is another df
little_fish
```
