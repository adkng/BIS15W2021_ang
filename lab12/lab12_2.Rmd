...
spiders %>% 
  ...
  summary()
```
We can see the min and max latitude + longtiude. We can use this to build our bounding box!

Now we set the bounding box.
```{r}
lat <- c(34.67, 41.80)
long <- c(-124.1, -115.5)
bbox <- make_bbox(long, lat, f = 0.05) # "f" means we are slightly expanding the box beyond the coordinates we provided... if we have stuff right on the edge, then it doesn't look very nice without this
```
...
```
"Stamen" is free and open source. Note, it takes a little bit of time to download because it is ammassing polygons to put together into a map. Don't fear!
...
```{r}
ggplot() + 
  geom_sf(data = us_comp, size = 0.125) + #size = width of lines defining boundaries
  theme_linedraw()+
  labs(title = "USA State Boundaries")
```
This doesn't work well for Hawaii + Alaska.
