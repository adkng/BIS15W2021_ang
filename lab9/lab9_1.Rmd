...
Plots are built in layers, and the grammar of graphics defines the layers.
...
One mistake people make is learning how to make extremely fancy plots... but really, it's overwhelming! Be a little humble when you make them for the best outcome.
...
```
You will end up with a blank plot... but look, everything is where it should be! We have a range, and the x/y axes we designated.
...
```{r}
ggplot(homerange,
       aes(x = log10.hra, y = log10.preymass)) + geom_point(na.rm = T) 
```
...
It just counts the number of times it sees a variable in a category.
...
homerange %>% 
  ...
  geom_bar()
```
*Tip: don't use pipes when specifying the geom_()... use `+` instead.*

To make your analysis of data clearer, consider adding both a table and graph! It helps you double check and makes your data easy to read.


### Bar Plot: `geom_col()`
Unlike `geom_bar()`, `geom_col()` allows us to specify an x-axis *and* a y-axis. ==> Use for specifying x and y axis only, while geom_bar() is for x axis only.
```{r}
homerange %>% 
  ...
  geom_col()
```
If you tried using geom_bar(), you will get an error because it only needs one axis specified!


However, you can still use geom_bar() in another fashion...
...
homerange %>% 
  ...
  group_by(trophic.guild) %>%
  count() %>% 
  ggplot(aes(y = trophic.guild, x = n)) + geom_col()
```
