...
We have to do a little data exploration to figure out how NAs are represented, where they are in the data, which later informs how we will treat them.
...
summary(msleep)
```
The summary function outputs the number of NAs in each variable... but remember, this is how **R** sees this... so if it was a different value, it probably couldn't tell the difference.
...
is.na(msleep)
```
This is a logical output for every cell in the data.

We also learned...
```{r}
any_na(msleep)
```
This just asks if there are any NAs, but not **where** they are.
...
```{r}
msleep %>% 
  summarise_all(~(sum(is.na(.)))) # is there any NAs in all the cells of the data frame?
```

## NA's using `purrr`
What if we are working with hundreds or thousands of variables?! In order to deal with this problem efficiently we can use another package in the tidyverse called `purrr`. We will talk more about `purrr` later, but for now let's have a look!
...
```
It's the exact same output as above!
...
```
This is a straight count and sorted by where they are most abundant... so easy to read! It is helpful to double check if the output makes sense.
...
```{r}
amniota %>% 
  summarise_all(~(sum(is.na(.)))) # why is there a tilde again?
```
...
*I would say turning the `-999.00` into physical NAs.*
...
*You hid your train of thought... not good! Just be transparent about it and show others how you fixed it in your codes.*
...
amniota_tidy %>% 
  ...
  arrange(desc(num_nas))
```
In this data, an `NA` means that the data doesn't exist **or** it was not recorded. Since there are a lot of NAs in the subspecies, we could infer that most of the animals in this data frame don't have a subspecies.
...
```
Very handy for replacing things in particular columns!
...
```{r}
life_history <- readr::read_csv("data/mammal_lifehistories_v2.csv") # fun fact: don't need the `readr::` call because the library is already loaded via tidyverse... just done for convention
```
...
```{r}
life_history %>% 
  skim() # see you could also do skimr::skim() and it would do the same thing
```
...
```{r}
life_history %>% 
  mutate(mass_mod = na_if(mass, -999.00)) %>% 
  count(mass_mod, sort = T)
```
```{r}
life_history %>% 
  count(gestation, sort = T)
```

```{r}
life_history %>% 
  mutate(gestation_mod = na_if(gestation, -999.00)) %>% 
  count(gestation_mod, sort = T)
```
```{r}
...
```{r}
life_history_tidy %>% 
  summarise(number_nas = sum(is.na(life_history_tidy)))
```

Questions: Why didn't this work after using the summarise function?
```{r}
life_history_tidy %>%
  na_if("-999") %>% 
  purrr::map_df(~ sum(is.na(.))) %>% 
