---
title: "Lab 7 Homework"
author: "Adrianna Ng"
date: "`r Sys.Date()`"
output:
  html_document:
    theme: spacelab
    toc: no
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## Instructions
Answer the following questions and complete the exercises in RMarkdown. Please embed all of your code and push your final work to your repository. Your final lab report should be organized, clean, and run free from errors. Remember, you must remove the `#` for the included code chunks to run. Be sure to add your name to the author header above.  

Make sure to use the formatting conventions of RMarkdown to make your report neat and clean!  

## Load the libraries
```{r message=FALSE, warning=FALSE}
library(tidyverse)
library(janitor)
library(skimr)
``` 

## Data
**1. For this homework, we will use two different data sets. Please load `amniota` and `amphibio`.**  

`amniota` data:  
Myhrvold N, Baldridge E, Chan B, Sivam D, Freeman DL, Ernest SKM (2015). “An amniote life-history
database to perform comparative analyses with birds, mammals, and reptiles.” _Ecology_, *96*, 3109.
doi: 10.1890/15-0846.1 (URL: https://doi.org/10.1890/15-0846.1).
```{r}
amniota <- read_csv("data/amniota.csv")
amniota <- clean_names(amniota)
```

`amphibio` data:  
Oliveira BF, São-Pedro VA, Santos-Barrera G, Penone C, Costa GC (2017). “AmphiBIO, a global database
for amphibian ecological traits.” _Scientific Data_, *4*, 170123. doi: 10.1038/sdata.2017.123 (URL:
https://doi.org/10.1038/sdata.2017.123).
```{r}
amphibio <- read_csv("data/amphibio.csv")
amphibio <- clean_names(amphibio)
```

## Questions  
**2. Do some exploratory analysis of the `amniota` data set. Use the function(s) of your choice. Try to get an idea of how NA's are represented in the data.**  

```{r}
amniota %>% 
  skim()
```
```{r}
summary(amniota)
```

**3. Do some exploratory analysis of the `amphibio` data set. Use the function(s) of your choice. Try to get an idea of how NA's are represented in the data.**  

```{r}
amphibio %>% 
  skim()
```
```{r}
summary(amphibio)
```

**4. How many total NA's are in each data set? Do these values make sense? Are NA's represented by values?**   

```{r}
amniota %>% 
  summarize(number_nas = sum(is.na(amniota)))
```
This doesn't make sense, because the `NA` value for this data frame is `-999.00`, of which I saw a lot... so there is definitely more than 0 missing data.

```{r}
amphibio %>% 
  summarize(number_nas = sum(is.na(amphibio)))
```
This does make sense, because when I looked through the data frame, the `NA` was represented as-is. However, I noticed that some columns have "0" in it, which I feel is another method of representing an `NA`, so there is some missing.

**5. Make any necessary replacements in the data such that all NA's appear as "NA".**   
```{r}
amniota_tidy <- amniota %>%
  na_if("-999") %>% 
  mutate(female_maturity_d_new = ifelse(female_maturity_d < 0, NA, female_maturity_d)) %>% 
  select(-female_maturity_d)
```

```{r}
amniota_tidy %>% 
  summarise(number_nas = sum(is.na(amniota_tidy)))
```

```{r}
amphibio_tidy <- amphibio %>% 
  na_if("0")
```
**Justification**: I read through the data and paper, and found that they used a binary unit to refer to "breeding strategy", which I interpreted as 1 = present and 0 = absent/not applicable. I figured that I should replace these `0` with `NA` because to me, this `0` indicates that the particular breeding strategy "does not apply" to this species (thus, fulfilling the `does not apply` of an `NA` definition).

```{r}
amphibio_tidy %>% 
  summarise(number_nas = sum(is.na(amphibio_tidy)))
```

**6. Use the package `naniar` to produce a summary, including percentages, of missing data in each column for the `amniota` data.**  
```{r}
amniota_tidy %>% 
  naniar::miss_var_summary()
```

**7. Use the package `naniar` to produce a summary, including percentages, of missing data in each column for the `amphibio` data.**
```{r}
amphibio_tidy %>% 
  naniar::miss_var_summary()
```
**8. For the `amniota` data, calculate the number of NAs in the `egg_mass_g` column sorted by taxonomic class; i.e. how many NA's are present in the `egg_mass_g` column in birds, mammals, and reptiles? Does this results make sense biologically? How do these results affect your interpretation of NA's?**  

```{r}
amniota_tidy %>% 
  group_by(class) %>% 
  select(egg_mass_g) %>% 
  naniar::miss_var_summary()
```
I see that the mammals have 100% NAs for this column based on the summary using `naniar`. This makes sense biologically because most mammals (save two) don't lay eggs. This would make me think that NAs here should be interpreted literally as "not applicable" to this category over "missing data that was not recorded".

**9. The `amphibio` data have variables that classify species as fossorial (burrowing), terrestrial, aquatic, or arboreal. Calculate the number of NA's in each of these variables. Do you think that the authors intend us to think that there are NA's in these columns or could they represent something else? Explain.**
```{r}
names(amphibio)
amphibio %>% 
  select(fos, ter, aqu, arb) %>% 
  naniar::miss_var_summary()
```
I think it is a little bit of both? Yes, there could be data that was not recorded or is missing because it was not observed when they recorded this data. However, I feel like the authors may have also used the NAs as a means of conveying that this species couldn't be classified as fossorial/terrestrial/aquatic/arboreal because it doesn't apply to the species.

**10. Now that we know how NA's are represented in the `amniota` data, how would you load the data such that the values which represent NA's are automatically converted?**
```{r}
amniota_advanced <-
  read_csv("data/amniota.csv", na = c("-999", "-30258.711"))
```
```{r}
summary(amniota_advanced)
```

## Push your final code to GitHub!
Please be sure that you check the `keep md` file in the knit preferences.  