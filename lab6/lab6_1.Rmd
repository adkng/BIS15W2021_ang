...
My work **Warning, this filter command is not what he was looking for... this will only look for the *string* NA!**
```{r}
msleep24 <- msleep %>% 
...
msleep24 <- msleep %>% 
  mutate(sleep_total_24 = sleep_total/24) %>% 
  select(name, vore, sleep_total_24) %>% 
  filter(!is.na(vore)) %>% # we ask R to filter everything that is *not* NA
  filter(sleep_total_24 <0.3) %>% 
...
skim(msleep24)
```
It gets basic information (number of rows, columns, classes), but a statistical summary with a mini-histogram! Very cool.
...
msleep24 %>% 
  tabyl(vore) %>%
  adorn_pct_formatting(digits = 1) # digits means how many numbers past the decimal
...
msleep %>%
  ...
  tabyl(order, vore)
```
A valid_percent column shows up to try and take care of the NAs... so filter it out!
...
I think the NA's mean that there are some species in the order that we do not have brainwt data for, so it cannot compute the mean.
...
msleep %>% 
  ...
  summarize(mean_brainwt = mean(brainwt, na.rm = TRUE),
            total = n())
```
Cetacea has an "NaN" entry, whereas Carnivora now has a mean calculated.
