...
## Vectors
Vectors are a common way of organizing data in R.  We create vectors using the `c` command. The `c` stands for concatenate. They are essentially a list of numbers or text. They are also ordered. Make your RMarkdowns as clean as possible, so space your vectors! 
...
```{r}
my_vector_sequence_extra <- c(1:100, seq = 0.5)
my_vector_sequence_extra
```
...
days_of_the_week[2]
```
We pulled out the "third element" in the days of the week vector.
If you pull "8", you get NA because it's not there!
...
2. Evaluate all numerics in `my_vector_sequence` to determine which has a value of 15. **Note: It can't ust be "=" because it would make another object and override the original to just "15'.**  
```{r}
...
4. If you use `[]` then you only get the values, not the logical evaluation of the entire vector. Experiment with this by adjusting the chunk below. **Note: Logical evaluation means TRUE/FALSE!**    
```{r}
yet_another_vector <- my_vector_sequence[my_vector_sequence <= 15]
```
...
Box office earnings for Harry Potter movies (in millions!). Notice that these are separate vectors for earnings in dollars (US versus INTL).  
```{r}
...
First, creating new object called harry_potter_matrix. In the matrix command, we want it to be arranged by row. So, there will be 8 different rows in the data matrix. You have to tell R how many rows you want there to be. The brackets are calling specific points in our vector. [1, _] is our first value in the first row. [_,1] is our first value in the column. Does not organize by columns (so, no ncol command).
...
What if we changed row numbers?
```{r}
harry_potter_matrix <- matrix(box_office, nrow = 4, byrow = T)
harry_potter_matrix
```
Without specifying, it will just give you as many rows as there are numbers. 
...
And even add a new column to reflect this calculation. `cbind()` adds columns. 'rbind()' adds rows.
```{r}
