...
```
There are about 37 good guys that don't have hair. 38 if you count Dr. Ledford.
```{r}
tabyl(bad_guys, hair_color)
```
There are about 35 bad guys that don't have hair. So, there are more good bald guys!
...
superhero_info %>%
  ...
  filter(weight > 450)
```
There are not 16 rows on #10 because the `!` operator returns the data that means condition 1 and/or condiion 2. Certain heroes fit both criteria (like Sasquatch, Wolfsbane), so they will show up when searching for either criteria separately.


## Height to Weight Ratio
13. It's easy to be strong when you are heavy and tall, but who is heavy and short (shortest and heaviest)? Which superheros have the highest height to weight ratio (tallest and skinniest)?
```{r}
superhero_info %>% 
  mutate(height_weight_ratio = height/weight) %>% 
  select(name, height_weight_ratio) %>% 
  arrange(desc(height_weight_ratio)) #gives tallest, skinniest
```
```{r}
superhero_info %>% 
  mutate(height_weight_ratio = height/weight) %>% 
  select(name, height_weight_ratio) %>% 
  arrange(height_weight_ratio) #gives shortest, heaviest
```
