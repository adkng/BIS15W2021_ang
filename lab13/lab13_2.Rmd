...
```
You can add this to your standalone app! If you type require and "janitor", it will do the same thing as library()... but if you use it as apart of an `if statement`, then it will install the thing for them if needed. This is good for people who are not often working in R!
...
server <- function(input, output, session) {
  session$onSessionEnded(stopApp) # how to make sure your app stops running when you say so (and it's not in the background)
...
ui <- fluidPage(
    ...
    sliderInput("fontsize", "Select the Font Size", min = 10, max = 24, value = 18, step = 1), # here is the slider!
...
server <- function(input, output, session) {
  ...
  ggplot(penguins, aes_string(x = input$x, y = input$y, color="species")) + 
      geom_point() + 
      theme_light(base_size = input$fontsize) #make sure you mention that text sliding!
  })
}
...
```{r}
library(shinydashboard)
# note, your app must have these things! it creates a pretty place to drop all the things you built
ui <- dashboardPage(
  dashboardHeader(),
  dashboardSidebar(),
  dashboardBody()
